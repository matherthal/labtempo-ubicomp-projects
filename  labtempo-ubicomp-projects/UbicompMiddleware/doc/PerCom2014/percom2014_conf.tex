...
In addition, end-users can compose context rules in a hi-level way by graphically combining the context variables from any RA and logical connectives, forming logical expressions as \textit{``if the TV from living room \textbf{is on}, \textbf{and} nobody has been present at the living room for 15 minutes, \textbf{then} turn off that TV''}. To compose this rule the user have to, first of all, select the rule composer mode in the [IPGAP]. Then, he must touch the TV icon at the living room representation on the map, and choose the desired context information -- in that case, the \textbf{ON} information. After that, a dialog box showing all possible logical operators is shown, so that the user can select one of them as the \textbf{AND} operator. 
