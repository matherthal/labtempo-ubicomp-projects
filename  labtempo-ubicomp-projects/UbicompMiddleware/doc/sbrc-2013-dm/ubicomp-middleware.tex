...
%possibilita a experimentação dos serviços de registro, descoberta e localização dos recursos, assim como o uso da interpretação de contexto, da simulação de ARs, da IPGAP, dentre outros. 

O Código 2 ilustra a inicialização do SmartLiC, onde são explorados os serviços de registro, localização, descoberta e a instanciação de regras de contexto. Uma vez obtidos os cômodos (ou lugares), a aplicação obtém cada lâmpada e sensor de proximidade deste cômodo. Uma vez com as identificações destes ARs e com o valor de T$_{max}$ definido, cria-se uma expressão para a regra (\mbox{\textit{ruleExpression}}) e define-se um conjunto de ações a serem desempenhadas (\mbox{\textit{actions}}).

% \begin{lstlisting}[caption=Inicialização do SmartLiC] 
%     List places = SLR.getPlaces(); \\Obter todos os lugares
%     for (place : places) { \\Para cada lugar...
%         \\Obter lâmpada
%         lampAR = SDR.search(TYPE, Lamp, place);
%         \\Obter sensor de proximidade
%         proxAR = SDR.search(TYPE, Proximity, place); 
%         \\Escrever expressão da regra de contexto
%         ruleExpression = lampAR.on and proxAR.occupied and T > Tmax;
% 	\\Criação de interpretador
% 	ContextInterpreter ci = new ContextInterpreter(ruleExpression)
% 	\\Associação de agente atuador
% 	new ResourceAgent() {
% 	    void action() {
% 		lampAR.turnOff();
% 	    }
...
%\begin{lstlisting}[caption=Inicialização do SmartLiC] 
   ...
   % for (place : places) { \\Para cada lugar...
       ...
       % \\Escrever expressão da regra de contexto
      %  ruleExpression = lampAR.on and proxAR.occupied and T > Tmax;
     %   \\Escrever conjunto de ações
    %    actions = lampAR.turnOff;
   %     \\Sistema se encarrega de instanciar IC e atuador
  %      newRule(ruleExpression, actions);
 %   } ...
%\end{lstlisting} 

% \begin{lstlisting}[caption=Interpretação] 
%     class ContextInterpreter {
% 	ContextInterpreter(ruleExpression) {
% 	    storedRule = parse(ruleExpression)
% 	    startEvaluation(storedRule)
% 	}
%     }
% \end{lstlisting} 

%Codigo do douglas
% \begin{lstlisting}[caption=Etapas de aplicação de regras do SmartLiC]
% 	define(rule, Lamp, Proximity);
% 	ruleFunction = compile(rule);
% 	List places = SLR.getPlaces();
% 	subscribeList(places, Lamp, Proximity);
% 	\\p/cada cômodo...	
% 	while(turnOn){
% 		receive(iAR,VC,value);
% 		updateStatus(iAR,VC,value);
% 		calculate(ruleFunction,place.status);
% 	}	
% \end{lstlisting}

% Código como uma rotina
% Rotina de criação de tratamento das regras de contexto do SmartLiC:
% \begin{enumerate}
%  \item Obter cômodos
%  \item Para cada cômodo:
%  \begin{enumerate}
%   \item Obter lâmpada e sensor de proximidade
%   \item Compor uma regra relacionando as VCs ``está ligada'' da lâmpada e ``está ocupado'' do sensor de proximidade
%   \item Definir um tempo T$_{max}$ para validação da regra
%   \item Criar um atuador para desligar a lâmpada do quarto e subscrevê-lo à regra
%  \end{enumerate}
% \end{enumerate}
% 
% Em cada IC:
% \begin{enumerate}
%  \item Recebe eventos da lâmpada e do sensor de proximidade
%  \item Avalia os eventos juntamente com a passagem de tempo
%  \item Se a regra foi validada, informar subscritores, senão volta a esperar
% \end{enumerate}
...
\begin{lstlisting}[caption=Inicialização do SmartLiC, mathescape] 
    for each place from SLR.getPlaces(): 
        lampAR = SDR.search(TYPE, Lamp, place)
        proxAR = SDR.search(TYPE, Proximity, place)
        ci = ContextInterpreter(lampAR.on $\land$ proxAR.occupied $\land$ T > T$_{max}$)
