...
Seção~\ref{subsec-comunicacao}. Os componentes (A, B, C, D) na Figura~\ref{fig:pubsub} correspondem aos ARs do AmbI. O uso das Operações ``publish'' e ``subscribe'' é explorado pelos Interpretadores de Contexto apresentados na Seção~\ref{subsubsec-ic}.
...
SDR & \textit{search} & Consulta & Busca ARs por tipo, local ou nome   \\ \hline
...
Na versão  SmartAndroid é inserido um AR do jogo que é associado ao Motor de um dos Jogadores (Jogador ``X''). O AR do Jogo intercepta a indicação de jogada deste Motor e encaminha para ARs de outros participantes (um deles o do Jogador ``O''). Este AR atualiza o tabuleiro com a jogada vinda por RPC do Jogador ``X'' (``atualiza'' do AR do Jogo). Para evitar que um jogador jogue na vez do outro, por exemplo, que o Jogador ``X'' jogue no turno do Jogador ``O'', é verificado se o Tipo do Jogador da vez é o mesmo do que tenta a jogada ("permissão"). Na Figura~\ref{fig:tictactoe} as ligações unidirecionais em amarelo indicam as alterações realizadas no processo de transformação. Usando as operações definidas na Tabela~\ref{tab:api}, o Código 1 apresenta a descoberta dos outros ARs na linha 1, o registro do AR local na linha 2, e a subscrição de interesses nas linhas 5 e 6.  Após isto, nenhuma alteração no nível de comunicação ou contexto foi necessária. Logo, podemos comprovar que o uso de AR tornou transparente 
