...
public class RuleConditionActivity extends Activity {
    ...
    private void populateClasses() {
    	...
    	/*FIXME: implement discovery
    	//Get all RAs
    	ArrayList<ResourceAgent> ras = discovery.search(""); 
    	//Iterator over the RAs to fill the spinner
    	Iterator<ResourceAgent> itr = ras.iterator();
    	ResourceAgent ra;
    	while (itr.hasNext()) {
    		ra = itr.next(); 
    		//If the RA class already exists in the spinner, don't add this again 
    		if (!ras.contains(ra.getType()))
    ...
    private void populateRA(String type) {
    	...
    	while (itr.hasNext()) {
    		...
    		if (ra.getType().equals(type))
    			adapter.add(ra.getName());
    	}
    	//Setting the adapter to the spinner of the RA's names
    	Spinner s = (Spinner) findViewById(R.id.spinnerRAChoice);
    	s.setAdapter(adapter);*/
    }
    ...
    private void populateAttrib(String type) {
    	...
    	//TODO: must get the methods from the interface that get the CVs. These methods also have to return the name of the CV
    	//PLACEHOLDER:
    	ArrayAdapter<CharSequence> adapter = new ArrayAdapter<CharSequence>(this, android.R.layout.simple_spinner_item);
    	...
    	/*ArrayAdapter<CharSequence> adapter = new ArrayAdapter<CharSequence>(this, android.R.layout.simple_spinner_item);
    	List<Tuple<String, Method>> attribs;
		try {
			attribs = ra.getAttribs();
	    	Iterator itr = attribs.iterator();
	    	while(itr.hasNext()) {
	    		Tuple<String, Method> tp = (Tuple<String, Method>) itr.next();
	    		adapter.add(tp.key);
	    	}
	    	//Setting the adapter to the spinner of the RA's attributes
	    	Spinner s = (Spinner) findViewById(R.id.spinnerRAAttribChoice);
	    	s.setAdapter(adapter);
		} catch (SecurityException e) {
			e.printStackTrace(); //TODO: deal with it
