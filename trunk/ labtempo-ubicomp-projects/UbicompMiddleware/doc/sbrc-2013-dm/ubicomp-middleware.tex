...
%TODO: Devo falar do JESS e outros motores de regras? Talvez..
...
for each place from SLR.getPlaces(): 
    lampAR = SDR.search(TYPE, Lamp, place)
    proxAR = SDR.search(TYPE, Proximity, place)
    config = (place, lampAR, proxAR)
    configList.add(config)
rule = Lamp.on $\land$ $\lnot$(Proximity.occupied) $\land$ T > T$_{max}$
ci = ContextInterpreter(configList, rule)

for each config from configList(): 
    Actuator(lamp.turnOff).subscribe(ci.config)
\end{lstlisting}
...
\begin{lstlisting}[caption=Interpretador de Contexto, mathescape]
ContextInterpreter(configList, rule):
    subscribeTo(configList, rule.getVCs()) 
    on event received (ar, vc, value): 
	  updateConfig(ar, cv, value)
	  validated = evaluate(rule, config)
	  if validated:
	      notifyActuator(config)
\end{lstlisting} 
