...
\begin{lstlisting}[caption=Inicialização do SmartLiC, mathescape] 
List comodos = SLR.getPlaces();
for each comodo from comodos: 
    lampAR = SDR.search(TYPE, Lampada, comodo)
    proxAR = SDR.search(TYPE, Proximidade, comodo)
	status = (comodo, lampAR, proxAR);
	statusList.add(status);
rule = Lampada.ligada $\land$ $\lnot$(Proximidade.ocupado) $\land$ T > T$_{max}$;		
IC = InterpretadorDeContexto(statusList, regra);
%Actuator(Lamp.turnOff).subscribe(ci)
\end{lstlisting}
...
\begin{lstlisting}[caption=Interpretador de Contexto, mathescape]
InterpretadorDeContexto(statusList, regra):
    subscribeTo(statusList, regra.VCs);\\Subscreve nas VCs dos ARs de cada cômodo
    on event received (AR, VC, valor):\\Novo valor de um VC de um AR
	  updateStatus(AR.local, VC, valor);\\Atualiza status do cômodo do AR  
	  status = getStatus(AR.local);	  
	  validated = evalute(rule, status.lampada.isOn(), status.proximidade.isOccupied());
	  if validated:
	      notifyActuators()
\end{lstlisting} 
