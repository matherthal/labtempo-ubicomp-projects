...
Besides, non-technical users are able to use [IPGAP] to manage their intelligent environment remotely using a mobile device such as a tablet. Through a intuitive GUI, he can not only visualize the state of their home devices but control them, for instance, turning off a stove burner. By touching the device in the screen, an interface that presents its main functions is shown, where the user can operate it as the tool were a remote control.
